...
def act(self, game_state: dict) -> str:
    ...
    # print(self.model.forward(game_state)[0])
    #print(F.softmax(self.model.forward(game_state)[0], dim=0).detach().numpy())
    res = np.random.choice(ACTIONS, p=F.softmax(self.model.forward(game_state)[0], dim=0).detach().numpy())
    # print(res)
    return res
    # return act_rule(game_state) # so this works nicely, does not throw bombs
