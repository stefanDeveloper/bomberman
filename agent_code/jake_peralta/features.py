...
def state_to_features(game_state: dict) -> np.array:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    hybrid_matrix = np.zeros((FEATURES_PER_FIELD, fx - 2, fy - 2), dtype=np.double)
    ...
    for (x, y), timer in game_state["bombs"]:
        hybrid_matrix[1, x - 1, y - 1] = timer

    # Coins
    for (x, y) in game_state["coins"]:
        hybrid_matrix[2, x - 1, y - 1] = 1
    hybrid_matrix[2] = hybrid_matrix[0].T

    # Crates
    hybrid_matrix[3, :, :] = np.where(game_state["field"][1:fx - 1, 1:fy - 1] == 1, 1, 0)
    ...
    hybrid_matrix[4, x - 1, y - 1] = 1
    hybrid_matrix[4] = hybrid_matrix[4].T
